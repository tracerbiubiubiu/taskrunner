...
func TestFireDueAdvancesNextRun(t *testing.T) {
	...
	st.CreateJob(context.Background(), repository.Job{
		JobID: "j1", ActionID: "audit_archive", TriggerType: repository.TriggerCron,
	...
	sub := &recSubmitter{}
	loop := &service.Loop{Store: st, Submit: sub, Logger: slog.Default(),
	...
	loop2 := &service.Loop{Store: st, Submit: sub, Logger: slog.Default(),
...
func TestSubmitFailureDoesNotAdvance(t *testing.T) {
	st, _ := repository.Open(filepath.Join(t.TempDir(), "t.db"))
	defer st.Close()

	past := time.Now().Add(-time.Minute)
	st.CreateJob(context.Background(), repository.Job{
		JobID: "j2", ActionID: "a", TriggerType: repository.TriggerCron, CronSpec: "* * * * *",
	...
	loop := &service.Loop{Store: st, Submit: sub, Logger: slog.Default(), NewTaskID: func() string { return "t" }}
	loop.FireDue(context.Background())
...
func TestNextRunDisabled(t *testing.T) {
	if _, ok := service.NextRun("* * * * *", false, time.Now()); ok {
		t.Fatal("disabled job should have no next_run")
	}
	if _, ok := service.NextRun("0 3 * * *", true, time.Now()); !ok {
