...
- **超时**：实现取 30s（env `TASKRUNNER_CALLBACK_TIMEOUT` 可改，载荷可按任务覆盖）——随 M3 验证后转正式口径（§10）；
...
**方案（待定）**：执行端把「能力唯一 `action_id` + 对应路由（callback_url + auth_key）」注册进 taskrunner DB（`capability_registry`），taskrunner 提交任务只认 `action_id`，自行查表解析回调目标。

- 表（taskrunner 侧 `internal/repository`，同 job_runs 领域 schema 口径，不放 zhuzhao-utils）：`action_id` PK（= 三层模型全局唯一 id）/ `callback_url` / `auth_key`（指向密钥环条目）/ `enabled` / `created_at` / `updated_at`；
- **注册**：执行端服务启动自注册 `POST /internal/actions`（upsert 幂等）+ 管理 / 配置兜底（避免启动顺序依赖）——能力服务（network 等）上报后即被 taskrunner 发现，提交方无需知道实现地址；
- **提交**：job 定义不再必填 `callback_url`，只填 `action_id`；提交时 taskrunner 查表解析出 `callback_url` + `auth_key` **快照入 job 行**（运行期不依赖目录在线，故障隔离、可追溯「当时调用了谁」）；显式 `callback_url` 保留覆盖能力（调试 / 直连 / 未注册目标）；
- **回调**：复用现有回调 client，按 job 行快照的 URL + auth_key 签名回调（AK/SK HMAC，§4 回调契约安全边界）；
- **两级路由**：跨服务目录（`action_id` → 服务端点，taskrunner 查）+ 服务内 `jobs.Registry`（`action_id` → Handler，执行端查）——各管一层、不重复；与 §4「能力服务（后置）」条目衔接（端点从"提交方手填"演进为"能力服务自注册"，taskrunner 仍只认 `action_id`）；
...
- 步骤 1（纯增量）：建 `capability_registry` 表 + 注册 API + 执行端启动自注册——提交仍手填，目录积累数据，两者互不干扰；
- 步骤 2（一行逻辑）：提交解析改为「显式 URL 优先 → 查目录 → 都无则 400」——新提交走目录，存量 job 定义靠已有 `callback_url` 照常跑；
- 步骤 3（可选清理）：存量定义逐个转纯 code 模式（或永不动，无害）；
- 全程无停机、无数据迁移、每步独立可回滚（步骤 2 的回滚 = 调用方恢复传显式 URL）；
- 唯一摩擦：语义重叠期（目录 vs 显式 URL 两个真相来源）——job 行快照存实际解析结果保证可追溯，配使用纪律（显式仅限调试/应急）即可。
...
| 2026-09-08 | **四路并行全面验证（4 子代理：核心域 / API 安全链 / 跨仓契约 / 构建运行时）**：核心 9 不变量全 PASS、跨仓契约无 P0（回调/提交字段·签名·幂等双栅栏全对齐）、门禁+race+双驱动等价+双模式冒烟 6/6 PASS。修复：① cron invalid-spec 真停用（原仅清 next_run，零值时间戳仍命中扫描 → 每 tick 重触发风险；新增 DisableJob）② TriggerJob 补 warning 日志（隐形任务第三入口）③ 日志轮转 env 绑定补齐 + MaxAge 默认 90 天（上轮编辑中断丢失）④ AKSKAuth 预鉴权 8MB 读体上限 ⑤ worker max_retry=0/未知边界（首败即 dead）⑥ jobs 索引补 SQLite ⑦ RowsAffected 不吞错 ⑧ readyz 不回内部错误细节 ⑨ params json.Valid→400 ×3 ⑩ retry 404 文案补 4xx 提示。补齐 worker 5 例 / service 干预竞态 6 例单测（首次覆盖） |
| 2026-09-09 | **二次审计修正**：① 上表④「AKSKAuth 预鉴权 8MB 读体上限」经查**实际未落地**（commit 未含 middleware.go）——本次补交付（MaxBytesReader 8MB + 413/400 分档，对齐 utils aksk.GinMiddleware 默认）；② worker 终态判定 off-by-one 修复（`attempt >= maxRetry` → `attempt > maxRetry`，asynq 语义 = 第 maxRetry+1 次失败才归档——原实现非末次失败提前写 dead 后被 MarkRunning 复活）；③ submit enqueue 增 asynq.Timeout（timeout_secs 上限 86400s，防超长超时占死 worker 槽位/阻塞停机——审计 M2）；handler 三入口补 timeout_secs 0–86400 校验。验证：build/vet/单测 5 包全绿 |
